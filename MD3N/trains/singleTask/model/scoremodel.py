...
def Euler_Maruyama_sampler(score_model,
    ...
    if condition is not None:
        channels, seq_len = condition.shape[1], condition.shape[2]
    else:
        channels, seq_len = 32, 48
    t = torch.ones(batch_size, device=device)
    if init_x is None:
        init_x = torch.randn(batch_size, channels, seq_len, device=device) \
                 * marginal_prob_std(t)[:, None, None]
    else:
        init_x = init_x.to(device) + init_noise_scale * torch.randn_like(init_x) * marginal_prob_std(t)[:, None, None]
    ...
    with torch.no_grad():
        for time_step in time_steps:
            ...
            if condition is not None:
                ...
                score = score_model(x, batch_time_step, perturbed_condition)
            else:
                score = score_model(x, batch_time_step)
            if guidance_fn is not None:
                scale = guidance_scale
                if guidance_schedule is not None:
                    scale = guidance_schedule(float(time_step.item()))
                if scale:
                    with torch.enable_grad():
                        guided_x = x.detach().requires_grad_(True)
                        guidance = guidance_fn(guided_x).detach()
                    score = score + scale * guidance
