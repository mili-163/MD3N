...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        feature_path = Path(self.args['featurePath'])
        if not feature_path.is_file():
            raise FileNotFoundError(
                f"Missing dataset file: {feature_path}. "
                "Download the aligned MOSI/MOSEI features or run `python train.py --smoke-test`."
            )
        ...
        text_dim = 768 if self.args.get('use_bert', False) else self.text.shape[2]
        self.args['feature_dims'] = [text_dim, self.audio.shape[2], self.vision.shape[2]]
